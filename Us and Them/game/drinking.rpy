label part_1:
    ...
    death_drive"{color=#808080}Do you want to know what could REALLY help in this situation?" (multiple=2)
    inner_child"Oh, god." (multiple=2)

    death_drive"Listen." (multiple=2)
    inner_child"{color=#808080}Oh, god." (multiple=2)

    death_drive"Listen listen listen..." (multiple=2)
    inner_child"{color=#808080}Oh, god." (multiple=2)

    death_drive"You could go for a drink right now." (multiple=2)
    inner_child"{color=#808080}Oh, god." (multiple=2)

    death_drive"You wanna know why?" (multiple=2)
    inner_child"{color=#808080}Oh, god." (multiple=2)

    menu:
        "\“Tell me.\”":
            death_drive"Because these are some strong things you’re feeling right now. You’re WAY too uncomfortable with what happened tonight." (multiple=2)
            inner_child"{color=#808080}Oh, god." (multiple=2)

            death_drive"You need to wash them out. Completely." (multiple=2)
            inner_child"{color=#808080}Oh, god." (multiple=2)

            death_drive"Like you did yesterday. And the day before that. And the day before that. And uh… ha, your memory these past four years has gotten so hazy." (multiple=2)
            inner_child"{color=#808080}Oh, god." (multiple=2)

        "\“I’m afraid of what you’ll have to say.\”":
            death_drive"{color=#808080}You wanna know why?" (multiple=2)
            inner_child"You should be. You sound like your father when you think things like that." (multiple=2)

            death_drive"Ha! Please, this has nothing to do with him." (multiple=2)
            inner_child"{color=#808080}You should be. You sound like your father when you think things like that." (multiple=2)

            death_drive"{color=#808080}Ha! Please, this has nothing to do with him." (multiple=2)
    ...
    death_drive"{color=#808080}{i}The winds of change were moving and in that small window of opportunity, it felt like history was going to be made BY Korea.{/i}" (multiple=2)
    ...
    inner_child"{color=#808080}{i}Not just that. There was pride in your comrades. They were proud of you.{/i}" (multiple=2)
    ...
    death_drive"{color=#808080}{i}You felt unstoppable. Like you could face whatever the world threw at you. You could capture the hearts and minds of the new nation with just a typewriter and your voice.{/i}" (multiple=2)
    inner_child"{i}But also, you felt like you belonged somewhere.{/i}" (multiple=2)

    death_drive"Yeah?" (multiple=2)
    inner_child"{color=#808080}{i}But also, you felt like you belonged somewhere.{/i}" (multiple=2)

    death_drive"{i}And where are those people now?{/i}" (multiple=2)
    inner_child"{color=#808080}{i}But also, you felt like you belonged somewhere.{/i}" (multiple=2)

    death_drive"{color=#808080}{i}And where are those people now?{/i}" (multiple=2)
    inner_child"{i}...they left.{/i}" (multiple=2)

    death_drive"Exactly." (multiple=2)
    inner_child"{color=#808080}{i}...they left.{/i}" (multiple=2)
    ...
    death_drive"{color=#808080}To escape the end of everything for just one evening." (multiple=2)
